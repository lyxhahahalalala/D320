...
static const uds_service_t uds_service_list[] =
{
	...
	{SID_27, SID_27_MIN_LEN, uds_service_27, FALSE, TRUE, FALSE, FALSE, UDS_SA_NON},//need change to level1 security when mp     
	...
	{SID_85, SID_85_MIN_LEN, uds_service_85, FALSE, TRUE, TRUE,  TRUE,  UDS_SA_LV1},
	...
	{SID_14, SID_14_MIN_LEN, uds_service_14, FALSE, TRUE, TRUE,  FALSE, UDS_SA_NON},
	{SID_19, SID_19_MIN_LEN, uds_service_19, FALSE, TRUE, TRUE,  FALSE, UDS_SA_NON},
...
static void uds_timer_stop (uint8_t num)
{
    if (num >= UDS_TIMER_CNT) return;
 ...
 */
static int uds_timer_run (uint8_t num)
{
 ...
 */
static int uds_timer_chk (uint8_t num)
{
    if (num >= UDS_TIMER_CNT) return 0;
 ...
 *     0 - ok, other - wrong
 */
static void uds_negative_rsp (uint8_t sid, uint8_t rsp_nrc)
{
 ...
 *     0 - ok, other - wrong
 */
static void uds_positive_rsp (uint8_t data[], uint16_t len)
{
    uds_timer_start (UDS_TIMER_S3server);
 ...
 *     bool
 */
static bool_t uds_check_len (uint8_t msg_buf[], uint16_t msg_dlc)
{
    bool_t result;
    uint8_t subfunction;
    uint8_t sid;
 ...
 *     void
 */
static void uds_service_10 (uint8_t msg_buf[], uint16_t msg_dlc)//诊断会话控制
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];
    ...
    switch (subfunction)
    {
	case UDS_SESSION_STD:  //0x01 DefaultSession：默认模式
	    curr_sa = UDS_SA_NON;
	    uds_session = (uds_session_t)subfunction;
	    uds_positive_rsp (rsp_buf, 6);
	    break;
	case UDS_SESSION_EOL:  //0x03 ExtendedDiagnosticSession：扩展模式
	    curr_sa = UDS_SA_NON;
	    uds_session = (uds_session_t)subfunction;
	    uds_positive_rsp (rsp_buf, 6);
	    uds_timer_start (UDS_TIMER_S3server);
	    break;
	case UDS_SESSION_PROG:  // 0x02 ProgrammingSession：编程模式
	 	#if 0
	 	rsp_buf[0] = USD_GET_POSITIVE_RSP (SID_10);
		rsp_buf[1] = subfunction;
		rsp_buf[2] = 0x00;
		...
		if(TRUE)
		{
			// update programe flag here all this should be with
	    ...
	    break; 
	case UDS_SESSION_SAFTY://0x04 SafetySystemDiagnosticSession：安全模式
 ...
 *     void
 */
static void uds_service_11 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];

    subfunction = UDS_GET_SUB_FUNCTION (msg_buf[1]);
    if ((subfunction == UDS_RESET_HARD)|| (subfunction == UDS_RESET_KEYOFFON)|| (subfunction == UDS_RESET_SOFT))
    {
 ...
 *     void
 */
static void uds_service_27 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];
    uint16_t i;
	...
	case UDS_SEND_KEY:
		...
		req_seed = 0;
		if (1)//(!uds_security_access (&msg_buf[2], org_seed_buf))
		{
		    rsp_buf[0] = USD_GET_POSITIVE_RSP (SID_27);
		    rsp_buf[1] = subfunction;
		    uds_positive_rsp (rsp_buf,2);
	...
	case UDS_REQUEST_SEED_SECURITY:
	    {
		if (uds_timer_chk (UDS_TIMER_FSA) > 0)
		{
		    uds_negative_rsp (SID_27,NRC_REQUIRED_TIME_DELAY_NOT_EXPIRED);
	...
	case UDS_SEND_KEY_SECURITY:
		...
		if (1)//(!uds_security_access (&msg_buf[2], org_seed_buf))
		{
		    ...
		    rsp_buf[1] = subfunction;
		    uds_positive_rsp (rsp_buf,2);
		    curr_sa = UDS_SA_LV1;//当前状态:已解锁，可以标定
		    //printf_os("SA_ PASS\r\n");
		}
		else
		{
 ...
 *     void
 */
static void uds_service_28 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];
    uint8_t cc_type;
...
/**
 * uds_service_3E - uds service 0x3E, TesterPresent 会话保持
 *
 * @msg_buf :
 * @msg_dlc :
 *
 ...
 *     void
 */
static void uds_service_3E (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    ...
    {
		if (!subfun_indication_bit)
		{      // only when the bit is false we send positive response
 ...
 *     void
 */
static void uds_service_85 (uint8_t msg_buf[], uint16_t msg_dlc)//用于停止或恢复诊断故障码的设置
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];
    subfunction = UDS_GET_SUB_FUNCTION (msg_buf[1]);
 ...
 *     void
 */
static void uds_service_22 (uint8_t msg_buf[], uint16_t msg_dlc)//客户端请求读取由数据标识符识别的某个记录的当前值
{
    ...
    for (msg_pos = 1; msg_pos < msg_dlc; msg_pos+=2)
    {
		...
		find_did = FALSE;
		if(((did >= 0x0112)&&(did <= 0x01A2)) || ((did >= 0x0320)&&(did <= 0x0321)))
		{
			for (did_n = 0; did_n < RWDATA_NUM; did_n++)
			{
				if (rwdata_list[did_n].did == did)
				{
					...
					readbuffer = EEROM_ReadWord(rwdata_list[did_n].ee_address);
					if(rwdata_list[did_n].dlc == 1)
						rsp_buf[rsp_len++] = (uint8_t)(readbuffer);
					else if(rwdata_list[did_n].dlc == 2)
					{
						rsp_buf[rsp_len++] = (uint8_t)(readbuffer);
						rsp_buf[rsp_len++] = (uint8_t)(readbuffer>>8);
					}
					else if(rwdata_list[did_n].dlc == 4)
					{
						rsp_buf[rsp_len++] = (uint8_t)(readbuffer);
						rsp_buf[rsp_len++] = (uint8_t)(readbuffer>>8);
 ...
 *     void
 */
static void uds_service_2E (uint8_t msg_buf[], uint16_t msg_dlc)//客户端请求写入由数据标识符指定的某个记录
{
    uint8_t rsp_buf[8];
    uint16_t did;
    uint16_t did_n;
	...
	if(((did >= 0x0112)&&(did <= 0x01A2)) || ((did >= 0x0320)&&(did <= 0x0321)))
	{
		...
		#if 0//在2e里面写   写入数据是正常的数据，写进去eeprom的排列是低位在前高位在后，intel格式，读出来时候需要转化
				if(rwdata_list[did_n].dlc == 1)
				writebuffer = msg_buf[3];
				else if(rwdata_list[did_n].dlc == 2)
				writebuffer = msg_buf[3] + msg_buf[4]*256;
				else if(rwdata_list[did_n].dlc == 4)
				writebuffer = msg_buf[3] + (msg_buf[4]<<8) + (msg_buf[5]<<16) + (msg_buf[6]<<24);
				else
				writebuffer = 0;//error,length not match
				write_result = EEROM_WriteEOL(rwdata_list[did_n].ee_address,writebuffer);
		#else//在31里面写
				write_result =1;
		#endif
				break;
	...
	else if((did >= 0xF110)&&(did <= 0xF1A6))
	{
		for (did_n = 0; did_n < LOGICDATA_NUM; did_n++)
	    {
			if (Logicdata_list[did_n].did == did)
			{
				...
				break;
			    find_did = TRUE;
			    if (msg_dlc != (Logicdata_list[did_n].dlc + 3))
				break;
			    vali_dlc = TRUE;
			    rsp_buf[0] = USD_GET_POSITIVE_RSP(SID_2E);
			    rsp_buf[1] = msg_buf[1];
			    rsp_buf[2] = msg_buf[2];
			    memcpy (Logicdata_list[did_n].p_data, &msg_buf[3], Logicdata_list[did_n].dlc);
...
/*----------------------------------------------------------------
  ...
  |SPN      				|FMI      |CM       | OC	 |
  ------------------------------------------
  |8-1bit | 8-1bit  |8-6bit  | 5-1bit|   8bit   |  7-1bit|
  ------------------------------------------
  |byte1  | byte2   |  b   y  t  e 3        |    b  y  t  e  4	 |
  ------------------------------------------
  ISO14229-1 DTCFomat
  ISO15031- 6 DTCFormat
  ---------------------------------------------
  |		DTC  					      | DTCStatus|
  ----------------------------------------------
  ...
  1	testFailedThisMonitoringCycle		当前监视循环报告该DTC处于故障状态
  2	pendingDTC						    DTC在当前或前一个监视循环处于故障状态
  ...
  6	testNotCompletedThisMonitoringCycle	本监视循环测试未完成
 ...
 *     void
 */
static void uds_service_14 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t rsp_buf[8];
    uint32_t dtc_group;
    dtc_group = 0;
 ...
 *     void
 */
static void uds_service_19 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    uint8_t rsp_buf[UDS_RSP_LEN_MAX];
	uint16_t dtc_dlc;
    ...
    switch (subfunction)
    {
	case REPORT_DTC_NUMBER_BY_STATUS_MASK://按照状态掩码读取DTC个数
	    {
		dtc_status_mask = msg_buf[2];//DTC状态掩码，即需要读取哪种状态的DTC
	...
	case REPORT_DTC_BY_STATUS_MASK://根据故障信息掩码读取DTC内容
	    {
	...
	case REPORT_DTC_SNOPSHOT_BY_DTC_NUMBER:
	    ...
	    dtcCode = (msg_buf[2]<<16) +  (msg_buf[3]<<8)  +msg_buf[4];
	    tempNode = get_dtcnode_by_code(dtcCode);
	    rsp_buf[0] = USD_GET_POSITIVE_RSP(SID_19);
	    rsp_buf[1] = subfunction;
		if(tempNode != NULL)
		{
			rsp_buf[2] = (uint8_t)(tempNode->DTCCode >> 16);
			rsp_buf[3] = (uint8_t)(tempNode->DTCCode >> 8);
			rsp_buf[4] = (uint8_t)(tempNode->DTCCode);
			rsp_buf[5] = (tempNode->DTCStatus.DTCStatusByte& DTC_AVAILABILITY_STATUS_MASK);
			//rsp_buf[6] =msg_buf[5];//DTC快照记录码DTCSnapshotRecordNumber，表示特定的 DTC快照数据记录编号
			/*DTCSnapshotRecordNumber表示DTC快照记录码，占一个字节，表示特定的 DTC快照数据记录编号。
			  例如当我们需要记录某个DTC第一次发生（假设用1表示）和最近一次发生的快照数据时（假设用2表示）
			  那么当DTCSnapshotRecordNumber为1时，则表示请求该DTC第一次发生时的快照信息。
			  如果ECU支持多个DTC快照数据记录，那么该纪录码应使用0x01~0xFE范围内的数值。
			  当该参数值为FF hex时，要求ECU一次性报告所有存储的DTC快照数据记录
			  */
			dtc_dlc  = get_dtc_snopshot_by_dtc_number (&rsp_buf[6], UDS_RSP_LEN_MAX-7, tempNode->SnapShotEEpromAddr,msg_buf[5]);
		}
		else
		{
			rsp_buf[2] = 0;
	    ...
	    uds_positive_rsp (rsp_buf, 7+dtc_dlc);
	    break;
 ...
 *     void
 */
static void uds_service_2F (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t rsp_buf[8];
 ...
 *     void
 */
static void uds_service_31 (uint8_t msg_buf[], uint16_t msg_dlc)
{
    uint8_t subfunction;
    uint8_t rsp_buf[8];
    uint16_t rid;
	...
	case UDS_ROUTINE_CTRL_START:
	    if (find_rid == TRUE)
	    {
			...
			{
				uds_negative_rsp (SID_31,NRC_REQ_REPET);//NRC_REQUEST_SEQUENCE_ERROR);
			}
			else
			{
				retval = rtctrl_list[rid_n].init_routine ();
				if(0x0202 ==rid)
				{
					if(0x00 == retval)
					{
 ...
 *     void
 */
extern void uds_get_frame (uint8_t func_addr, uint8_t frame_buf[], uint8_t frame_dlc)
{
