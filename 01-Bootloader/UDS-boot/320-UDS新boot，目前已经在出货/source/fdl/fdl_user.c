 ...
 * Mod. Date     : $Date: 2014/09/10 16:23:12MESZ $
...
#if APP_COMPILER == APP_COMP_IAR
    asm( "mov     PSWSafe, r6" );                       /* backup PSW */
#else 
...
void FDL_User_CriticalSetionEnd( void )
{
    asm( "ldsr    r0, 31" );                            /* select system register bank 0 
                                                          (contains PSW)*/
#if APP_COMPILER == APP_COMP_IAR
    asm( "mov     PSWSafe, r6" );                       /* Restore PSW value */
#else 
    asm( "mov     _PSWSafe, r6" );                      /* Restore PSW value */
#endif    
    asm( "ld.w    0[r6], r7" );                         /* " */
    asm( "ldsr    r7, 0x05" );                          /* write PSW */
}
#else
