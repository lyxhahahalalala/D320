...
uint8 BT_numAlHandles_u8 = sizeof(BT_WorkConf_cs) / sizeof(BT_WorkConf_t);
...
static uint8 BT_stAddrMode_u8;   /* Current active address mode */
...
static uint32  BT_dReqTim_au8[NUMTIMINGS];
/* Byte Register to store the change requests */
uint8  BT_stChgReq_u8;
...
void BT_DoChanges_Proc ( void )
{
    ...
    if (BT_stCallDoChg_u8 != 0)
    {
        /* Check if the previous frame has been transmitted succesfully */
        if (BT_TxSt == 1)
        {
            BT_DoChanges();
            /* Inside BT_DoChanges after making
               the required changes by the service the variable BT_calldoChanges will
               be resetted */
        }
    }
}
...
void BT_DoChanges(void)
{
    ...
    if (BT_stChgReq_u8 > 0)
    {
        ...
        if  ((BT_stChgReq_u8 & BT_TIMCHGREQ) > 0 )
        {
            ...
            BT_stChgReq_u8 &= (uint8)(~BT_TIMCHGREQ);
        }
        /* Execute the given function */
        if ((BT_stChgReq_u8 & BT_EXECFUNCREQ) != 0 )
        {
            /* reset function flag */
            BT_stChgReq_u8 &= (uint8)(~BT_EXECFUNCREQ);
...
bool BT_TpTxmsg (uint8* databuff_pu8, uint16  datalength_u16 )
{
    ...
    if (BT_tpMsgCtrl_ps->tp_state_u8 == TP_ST_IDLE)
    {
        ...
        BT_tpMsgCtrl_ps->buffer_pu8 = databuff_pu8;
        /* lock the buffer by the network layer */
        BT_tpMsgCtrl_ps->bufferstate_u8 = TP_STBUFF_NL;
        BT_tpMsgCtrl_ps->datatoRxTx_u16 = datalength_u16;
        ...
        if (BT_tpMsgCtrl_ps->datatoRxTx_u16 > candataLen_u8)
        {
            /* Multiple frame transmission */
            *buffer_pu8++ = N_PCI_FF | ((datalength_u16 >> 8) & 0x0F);
            *buffer_pu8++ = (uint8)(datalength_u16 & 0xff);
            ...
            candataLen_u8--;
            BT_TxPdu.length = candataLen_u8 + 2;

        }
        else
        {
            /* Single frame transmission */
            *buffer_pu8++ = (uint8)(N_PCI_SF | datalength_u16);
            candataLen_u8 = (uint8)datalength_u16;
...
void BT_TpRxmsg (uint8 msgObjIdex, uint8 dlc_u8, uint8* data_pu8 )
{
    uint8* buffer_pu8;
    uint8 buffer_au8[8];
    uint16 idx_u16;
    uint16 datalenFF_u16;
    uint8 address_u8;
    uint8 Nl_handle_u8;
    uint8 numPciTyp_u8;
    uint8 numPcilownib_u8;
    uint8 dataLen_u8;
    buffer_pu8 = buffer_au8;

    for ( idx_u16 = 0; idx_u16 < dlc_u8; idx_u16++)
        /* Copy the data from the CAN buffer to the
           local buffer to avoid overwritting of the buffer by the CAN driver software*/
    {
    ...
    if ((BT_tpMsgCtrl_ps->tp_state_u8 == TP_ST_SENDCF))
    {
    ...
    numPciTyp_u8 = (*buffer_pu8) & (0xf0);
    numPcilownib_u8  = (*buffer_pu8++) & (0x0f);

    if (BT_tpMsgCtrl_ps->tp_state_u8 == TP_ST_WAITFC )
    {
        if (numPciTyp_u8 != N_PCI_FC)
        {
            return;/* Transmit in progress ignore the unexpected frame */
        }
    }
    Nl_handle_u8 = BT_GetMsgHandle(msgObjIdex);
    address_u8 = BT_GetAddrType(msgObjIdex);
    if (address_u8 == ADRM(FUNC))
    {
        if ( numPciTyp_u8 != N_PCI_SF )
        {
            /* ignore the frame */
            return;
        }
    ...
    case N_PCI_SF:
        ...
        if ((numPcilownib_u8 == 0) || (numPcilownib_u8 > dataLen_u8))
        {
    ...
    case N_PCI_FF:
        ...
        datalenFF_u16 = (numPcilownib_u8 << 8) + *buffer_pu8++;
        ...
        if (datalenFF_u16 <= (uint16) dataLen_u8)
        {
            return ;
        }
        ...
        BT_tpMsgCtrl_ps->datatoRxTx_u16 = datalenFF_u16;
        BT_tpMsgCtrl_ps->bufferindex_u16 = 0;

        /* Reset the WFC counter */
        BT_tpMsgCtrl_ps->wfcCtr_u8 = 0;
        if (BT_tpMsgCtrl_ps->datatoRxTx_u16 <= BT_tpMsgCtrl_ps->bufferlength_u16)
        {
        ...
        else
        {
            ...
            BT_tpMsgCtrl_ps->tp_state_u8 = TP_ST_SENDFC_OVR;
        }
        break;
    ...
    case N_PCI_CF:
        ...
        if (BT_tpMsgCtrl_ps->tp_state_u8 == TP_ST_WAITCF)
        {
            if (numPcilownib_u8 == BT_tpMsgCtrl_ps->seqCtr_u8 )
            {
                ...
                if (BT_tpMsgCtrl_ps->datatoRxTx_u16 <= ((BT_tpMsgCtrl_ps->bufferindex_u16) + dataLen_u8))
                {
                    /* Last Frame Received */
...
void BT_TpInd (uint8 Nl_handle_u8, uint16  dataLength_u8, uint16 Nl_Result_u16)
{
    uint8   idx_u8;
    ...
    {
        for (idx_u8 = 0; ((idx_u8 < (uint32)BT_numAlHandles_u8)
                          && (stSearch_b == (bool)FALSE)); idx_u8++ )
        {
        ...
        if (stSearch_b != (bool)FALSE)
        {
            ...
            BT_Status_setNumUsed(dataLength_u8);
    ...
    else if ((Nl_Result_u16 == TP_TIMEOUT_Cr) || (Nl_Result_u16 == TP_WFT_OVRN) || (Nl_Result_u16 == TP_UNEXP_PDU) || (Nl_Result_u16 == TP_WRONG_SN))
    {
 ...
 *********************************************************************
 */
void BT_TpConfirm (  uint16 Nl_Result_u16 )
{
    if (Nl_Result_u16 == TP_OK)
    {
 ...
 *********************************************************************
 */
void BT_TpSetBuffer( uint8* data_pu8, uint16 bufferlength_u16 )
{
...
void BT_CopyData(uint8* src_pu8, uint8* dest_pu8, uint8 len_u8)
{
    while (len_u8 > 0)
    {
        *dest_pu8++ = *src_pu8++;
...
void BT_Tp_Proc ( void )
{
    ...
    if (((sint32)(BT_tpMsgCtrl_ps->timer_u32 - getCurrentTime) <= (sint32)(0x00) ) && (BT_tpMsgCtrl_ps->tp_state_u8 != TP_ST_IDLE))
    {
        ...
        case TP_ST_SENDCF:
            ...
            else
            {
                ...
                *buffer_pu8++ = N_PCI_CF | BT_tpMsgCtrl_ps->seqCtr_u8;
                /* check if last frame needs to be transmitted */
                if ( (BT_tpMsgCtrl_ps->bufferindex_u16 + numBytesTx_u8) >= BT_tpMsgCtrl_ps->datatoRxTx_u16)
                {
                    /* last frame transmitted */
                    numBytesTx_u8 = (uint8)(BT_tpMsgCtrl_ps->datatoRxTx_u16 - BT_tpMsgCtrl_ps->bufferindex_u16);
...
uint8 BT_GetAddrType(uint8 MsgObjIdx)
{
    uint idx_ui;
    const BT_WorkConf_t* workConf_pcs;
    workConf_pcs = &BT_WorkConf_cs;

    for (idx_ui = 0; idx_ui < (uint32)BT_numAlHandles_u8; idx_ui++ )
    {
    ...
    return ((uint8)(workConf_pcs[idx_ui].addrmodes_u8));

}

uint8 BT_GetMsgHandle(uint8 MsgObjIdx)
{
    uint idx_ui;
    const BT_WorkConf_t* workConf_pcs;
    workConf_pcs = &BT_WorkConf_cs;
    ...
    return ((uint8)(workConf_pcs[idx_ui].Nl_handle_u8));
...
void BT_SetAddrMode(uint8 stNewMode_u8)
{
