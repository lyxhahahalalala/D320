...
int main (void)
{
    ...
    if((canUpdate ==0) &&( (resf<<31) == 0) )               //no key pressed and no software reset  Read_Value == 0
    {
		__DI();
		//  ExecutiveAppHandle();
		Src = (uint8_t *)(0x10000);
...
void FCL_Erase()
{
    ...
    R_FCL_Execute (&myRequest);
    block_num = readBuffer_u32[0];

    /* Read out the current lock bit setting */
    myRequest.command_enu   = R_FCL_CMD_GET_ERASE_PROTECT_FLAG;
    myRequest.bufferAdd_u32 = (uint32_t)&readBuffer_u32[0];
    R_FCL_Execute (&myRequest);
    block_num = readBuffer_u32[0];//lock bit
    if(block_num!= 0)
    {
    }
#endif
    ...
    myRequest.idx_u32     = 0x8;                   /* erased range = 0x10000 to end */
    myRequest.cnt_u16     = 118;
    R_FCL_Execute (&myRequest);
#if R_FCL_COMMAND_EXECUTION_MODE == R_FCL_HANDLER_CALL_USER
    while (R_FCL_BUSY == myRequest.status_enu)
    {
