...
r_Error_t Clock_Init( void )
{
	...
	set_clock_domain(CPU_Clk_4, CPU_PLL0, FIX_PLL1CLK_6, FIX_ClkDivSel, JIT_PLL0CLK_6, JIT_ClkDivSel);          /* Domain clock setting      */
	return retVal;
}
...
static r_Error_t osc_clock(uint32 FrqIn, uint32 Pll0Out, uint32 Pll1Out)
{
	...
	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD0, MOSCE, 0x01u);	/*SYSMOSCE*/
	}while(R_DEV_READ_REG(32, LOC_PROTS0)==1);			/* trigger enable (protected write) */
...
/*	PLL0 output frequency configuration	*/
	...
	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, PLL0E, 0x01);		/*SYSPLL0E*/
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);		 /* trigger PLL0 enable (protected write) */
...
/*	PLL1 output frequency configuration	*/
	...
	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, PLL1E, 0x01);		/*SYSPLL1E*/
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);		 /* trigger PLL1 enable (protected write) */
...
static void set_clock_domain( CPU_ClkDiv CpuDiv, CPU_ClkSel CpuSel, FIX_ClkDiv FixDiv, FIX_ClkSel FixSel, JIT_ClkDiv JitDiv, JIT_ClkSel JitSel)
{
    /* set CPU clock divider to 4 */
 	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, CPUCLKD, CpuDiv);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	while(CpuDiv!=R_DEV_READ_REG(32, CPUCLKD_ACT));

    /* set CPU clock to PLL0CLK */
	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, 0xFFF8A000, CpuSel);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	...
	/* set clock divider for CLKFIX to 6 (480MHz / 6 = 80 MHz)*/
 	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, FIXCLKD, FixDiv);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	...
	/* set CLKFIX to PLLFIXCLK / 6 (80 MHz) */
 	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, FIXCLKS, FixSel);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	...
	/* set CLKJIT divider to PLL0 / 6 (80 MHz) */
 	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, JITCLKD, JitDiv);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	...
	/* set CLKJIT to CKDV_ICLKJITD_CTL (80 MHz) */
 	do
	{
		R_DEV_WRITE_PROT_REG(LOC_PROTCMD1, JITCLKS, JitSel);
	}while(R_DEV_READ_REG(32,LOC_PROTS1)==1);
	while(0x0==R_DEV_READ_REG(32, JITCLKS_ACT));
}
