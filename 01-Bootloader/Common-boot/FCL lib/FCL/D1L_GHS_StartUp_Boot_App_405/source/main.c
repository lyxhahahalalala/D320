...
void main(void) 
{
	...
	SET_EBASE(0x00010000); //set Ebase to 0x10000, set RINT to 1
	SET_PSW_EBV();
	init_TAUB0();
	
        Port_Init_Simple(PORT42, PIN15, Normal_Port, Pin_Out);

	__EI();                               /* enable interrupts globally */
	while(1)
	{
	     //PORT_ISO.P42.BIT.P42_15 = ~ PORT_ISO.P42.BIT.P42_15 ;/* endless loop */
	    // delay();
	}
}
...
void init_Gpio(void)
{
	...
	Port_Init_Simple(PORT42, PIN15, Normal_Port, Pin_Out);	/*P17_6, port mode, output*/
