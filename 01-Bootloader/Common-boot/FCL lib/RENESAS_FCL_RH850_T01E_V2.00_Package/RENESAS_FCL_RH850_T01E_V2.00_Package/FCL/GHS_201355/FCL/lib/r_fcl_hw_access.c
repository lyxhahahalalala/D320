...
* Lib. Version  : $RH850_FCL_LIB_VERSION_T01: E2.10c $
* Mod. Revision : $Revision: 1.88 $
* Mod. Date     : $Date: 2015/11/09 09:34:22MEZ $
...
#if !(defined R_FCL_NO_BFA_SWITCH)
...
/*********************************************************************************************************************/
    ...
    #pragma section text "R_FCL_CODE_RAM"
#endif
uint32_t R_FCL_I_GetCPUCorePid (void)
{
    uint32_t pid;
...
R_FCL_STATIC r_fcl_status_t R_FCL_FCUFct_Chk_DBFull (void)
{
    ...
    res = R_FCL_FCUFct_ChkReg (R_FPSYS_REGADD_FSTATR_U32, 
                               ...
                               0x00000000uL,                /* Wait until the DBFULL bit is cleared */
...
void R_FCL_FCUFct_PrepareEnvironment (void)
{
    ...
    if (g_fcl_data_str.opVar_str.opStatus_enu == R_FCL_OP_PREPARE_STATE)
    {
        ...
        if (R_FPSYS_REGBIT_FENTRY_OFF == regFENTRYR)
        {
            ...
            #ifdef R_FCL_NO_BFA_SWITCH
                /* Get device information - must be read outside BFA switch for F1K*/
                R_FCL_FCUFct_GetDeviceInfo ();
            #endif
                                      
            /* Activate BFlash
               Note: required even for R_FCL_NO_BFA_SWITCH configuration. See function header of R_FCL_FCUFct_GetDeviceInfo */
            R_FCL_FCUFct_Switch_BFlash (0x01u);
            ret = R_FCL_FCUFct_Clear_Cache ();
 ...
 *       only once (for devices with and without R_FCL_NO_BFA_SWITCH set) in prepare environment. It is not separately 
 *       handled for R_FCL_NO_BFA_SWITCH set
 *
 * @param[in,out] -
 * @return        ---
 */
