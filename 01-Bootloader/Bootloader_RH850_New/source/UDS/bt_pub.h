...
#define BT_ADDITIONAL_PLACE (BT_MAX_HEADER_LENGTH - BT_MIN_HEADER_LENGTH + 1)  //0x04
...
#define BT_RESP_PEND             ((uint8_t)0x1)
#define BT_NO_RESP_PEND          ((uint8_t)0x0)
#define BT_MUL_RESP              ((uint8_t)0x1<<1)
#define BT_NO_MUL_RESP           ((uint8_t)0x0)
...
#define ADRM_BM(mode)   (uint16_t)BT_ADRM_##mode##_DB16
...
typedef struct
{
    uint32_t  timer_u32;        //定时器           /* Timer */
    uint8_t*  buffer_pu8;       //缓冲区指针       /* Buffer pointer */
    uint16_t  bufferindex_u16;  //缓冲区索引       /* Buffer index */
    uint16_t  bufferlength_u16; //缓冲区长度       /* Buffer length */
    uint16_t  datatoRxTx_u16;   //接收和发送的数据 /* Data to Receive and Transmit */
    uint8_t   bufferstate_u8;   //缓冲区状态       /* Buffer State */
    uint8_t   tp_state_u8;      //网络层状态       /* Network layer state */
    uint8_t   wfcCtr_u8;        //等待发送流控制   /* Wait to send flow Control */
    uint8_t   seqCtr_u8;        //序列计数器       /* Sequence Counter */
    uint8_t   BSmaxCtr_u8;      //Bsmax计数器      /* Bsmax Counter */
    uint8_t   srcaddr_u8;       //计算SRC地址      /* calc the src addr */
    uint8_t   stmin_u8;         //启用 Stmin 时间  /* active Stmin timer */
    uint8_t   BSmax_u8;         //BSmax 值         /* BSmax value */
} BT_tpMsgCtrl_t;
...
typedef struct
{
    uint8_t  Nl_handle_u8; /* Mapped NL handle */
    uint8_t  RxMsgObjIdx;  /* Receive message object index */
    uint8_t  addrmodes_u8; /* Addressing mode */
} BT_WorkConf_t;
...
typedef enum
{
    P2MIN,      /*请求和响应之间的最短时间*/ /* Minimum time between the request and the response */
    P2MAX,      /*请求与响应之间的最大时间*/ /* Maximum time between the request and the response */
    P3MIN,      /*从响应到请求的最短时间*/   /* Minimum time between the response and the request */
    P3MAX,      /*响应和请求之间的最大时间*/ /* Maximum time between the response and the request */
    S3MAX,      /*会话最大超时时间*/         /* Maximum time for Session Timout */
    NUMTIMINGS,
} BT_alTiming_e;

/* Array of timings */
typedef uint32_t BT_alTiming_t[NUMTIMINGS];
...
typedef struct
{
    uint8_t* dBuf_pu8;         //缓冲区仅用于数据 /* buffer only for data */
    uint16_t dBufSize_u16;     //缓冲区大小       /* size of buffer */
    uint16_t numBytesReq_u16;  //请求长度         /* request's length */
    uint16_t numBytesResp_u16; //响应长度         /* response's length */
    uint8_t  stResp_u8;        //响应类型         /* response's type */
    uint8_t  dSID_u8;          //请求的SID        /* request's SID */
    uint8_t  dFirstPara_u8;    //请求的第一个参数 /* request's first parameter */
    bool     dMulRespAllow_b;  //允许多重响应机制 /* multiple response mechanism allowed */
} BT_ServiceMsg_t;
...
typedef struct
{
    uint8_t dSID_u8;       /* KWP2000 service identifier */
    uint8_t dFirstPara_u8; /* First parameter of message (behind SID) */
    uint16_t dOpMode_u16;  /* Operating modes where service is allowed */
    void  (*Service_p)( BT_ServiceMsg_t*); /* Pointer to service interpreter */
} BT_ServiceTab_t;
...
{
    uint8_t* dUsed_pu8;
    uint16_t numUsed_u16;
    bool     stRxOk_b;
    bool     stRxHeaderOk_b;
    bool     stReceiving_b;
    bool     stTxOk_b;
    uint8_t  stAddrMode_e;
    uint8_t  stAddress_u8;
    uint8_t  stTesterAddr_u8;
} BT_st_t;
...
typedef struct
{
    uint8_t  FmtBits_u8;     //头中的格式位                 /* FormatBits in Header */
    uint8_t  unused1;        //填补空白                     /* fill gap */
    uint8_t  unused2;        //填补空白                     /* fill gap */
    uint8_t  unused3;        //填补空白                     /* fill gap */
    uint8_t* MsgBuf_pu8;     //指针指向缓冲区中头文件的开始 /* pointer to start of header in buffer */
    bool     AddrExists_b;   //头，地址信息                 /* header with address infos */
    uint8_t  EcuAddr_u8;     //报文中的Ecu地址              /* Ecu address in telegram */
    uint8_t  TesterAddr_u8;  //报文中的测试者地址           /* tester address in telegram */
    bool     LenExists_b;    //头加上额外长度的字节         /* header with additional length byte */
    uint8_t  HdrLength_u8;   //报文头长度                   /* header length in telegram */
    uint8_t  DataLength_u8;  //报文中的数据长度             /* data length in telegram */
    uint8_t  unused4;        //填补空白                     /* fill gap */
    uint8_t  unused5;        //填补空白                     /* fill gap */
    uint8_t* DataBuf_pu8;    //指针指向缓冲区中数据的开始   /* pointer to start of data in buffer */
} BT_HdrInfo_s;
...
typedef struct
{
    ...
    uint8_t data [BT_WARM_RESPONSE_MAX_LENGTH];
...
typedef struct
{
    ...
    uint32_t activeP2min;
    uint32_t activeP2max;
    uint32_t activeP3min;
    uint32_t activeP3max;
    uint32_t activeS3max;
...
typedef struct
{
    uint8_t  addrExists_b: 1; /* header with address infos          */
    uint8_t  lenExists_b : 1; /* header with additional length byte */
    uint8_t  fmtbits_b2 : 2;  /* FormatBits in Header               */
} BT_StateBitFields_t;
...
{
    /* previous active Timings in ticks      */
    uint32_t activeP2min_u32;
    uint32_t activeP2max_u32;
    uint32_t activeP3max_u32;
    uint32_t activeP4min_u32;
	
    uint32_t activeBaudrate_u32;    /* previous active Baudrate 
	                                   in Baud (only relevant for K-Line) */
    uint32_t reqBdRate_u32;         /* requested baudrate for baudrate change 
	                                   after response is sent */
    uint32_t freeForProjectUse_u32; /* one uint32_t can be used 
	                                   e.g. to store pointer to additional structure */
    uint8_t  internInitRequ_u8;     /* Signalises a shorted Initialisation for switch
                                     * from one CustomerBlock to another, if req==0x77 , oth==0 */
    
	uint8_t  activeComMedium_u8;    /* 0..K-Line,
                                     * 1..CAN,
                                     * 2..ETK,
                                     * 3..CAN-CAN Gateway,
                                     * 4..CAN-KLine-Gateway */
    uint8_t  activeComProtocol_u8;  /* Communication Protocol:
                                     * 0..KWP2000(K-Line),
                                     * 1..KWP2000*(K-Line),
                                     * 2..KWP2000(CAN) */
    
	uint8_t  ecuAddr_u8;            /* physical ECU address */
    uint8_t  changeReq_u8;          /* Required for changing of Baudrate */
    uint8_t  actStimuAddr_u8;       /* previous active stimulation Address */
    uint8_t  actStimuAddrMode_u8;   /* previous active stimulation Address */
    uint8_t  diagMode_u8;           /* previous active diagnostic mode*/
    uint8_t  secLevel_u8;           /* previous active security level */
	
    BT_StateBitFields_t bitfields_s;/* Com state bit fields contains addrExists_b,
	                                   lenExists_,fmtbits_b2 */
	
    uint8_t  testerAddr_u8;         /* tester address in telegram */
    uint8_t  hdrLength_u8;          /* header length in telegram */
    uint8_t  addrId_u8;             /* index of dcan address table */
	
    uint8_t  freeForProjectUse0_u8; /* For future expansion */
    uint8_t  freeForProjectUse1_u8; /* For future expansion */
    uint8_t  freeForProjectUse2_u8; /* For future expansion */
    uint8_t  freeForProjectUse3_u8; /* For future expansion */
	
	/* K2Sess state machine: transmit on warm reset. */
	/* one byte can be used e.g. to store service state for reinitialization */
    uint8_t   warmResetTransmit_b; 
	
    uint8_t warmRespDatalength_u8; /* The length of the response: SID + data. */
	
    /* The response to be send to tester on warm reset. */
    /* The SID  + data to be send to tester on warm reset. */
    uint8_t warmRespData_au8 [BT_WARM_RESPONSE_MAX_LENGTH];
 ...
 */
/* Function reference */
extern void BT_TpRxmsg (uint8_t msgObjIdex, uint8_t dlc_u8, uint8_t* data_pu8 );
...
/* Function to set the Exec function which will be called after the transmission of the response */
extern void BT_ExecFunction(void (*fct)(void));

/* Declare routine active/inactive */
extern void  BT_SetRoutine(uint32_t dRoutId_b32, uint8_t stMode_b8);

extern void  BT_SetSecLevel(uint8_t dSecLevel_u8);
extern bool  BT_GetPerm(uint32_t dProtection_b32);
...
extern uint8_t BT_GetAddrType(uint8_t MsgObjIdx);
extern uint8_t BT_GetMsgHandle(uint8_t MsgObjIdx);
extern void BT_SetAddrMode(uint8_t stNewMode_u8);
extern uint8_t BT_GetAddrMode(void);
/* The functions should be called in the same order as mentioned below by the OS*/
...
/* application layer interface functions. Process task call. */
