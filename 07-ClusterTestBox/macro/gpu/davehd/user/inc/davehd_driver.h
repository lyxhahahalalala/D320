 ...
 *  $Revision: 4997 $
 *  $Date: 2015-03-31 14:53:23 +0200 (Di, 31 Mrz 2015) $
...
/*----------------------------------------------------------------------------------------------------------*/
#define DHD_PREMULTIPLY_LIMIT_RGB  256         /* additional flag to that can be or'ed to dhd_pixel_org_t or dhd_texel_org_t enums*/
...
typedef enum tagdhd_drawattribute_flag {  
  ...
  E_DHD_DRAW_UV         = DHD_DRAW_UV(1),         /* drawing should use one set of UV corrdinates per primitive*/
  E_DHD_DRAW_1UVS       = DHD_DRAW_UV(1),         /* drawing should use one set of UV corrdinates per primitive*/
  E_DHD_DRAW_2UVS       = DHD_DRAW_UV(2),         /* drawing should use two sets of UV corrdinates per primitive (texture units 0..1)*/
  E_DHD_DRAW_3UVS       = DHD_DRAW_UV(3),         /* drawing should use three sets of UV corrdinates per primitive (texture units 0..2)*/
  E_DHD_DRAW_4UVS       = DHD_DRAW_UV(4),         /* drawing should use four sets of UV corrdinates per primitive (texture units 0..3)*/
  E_DHD_DRAW_5UVS       = DHD_DRAW_UV(5),         /* drawing should use fife sets of UV corrdinates per primitive (texture units 0..4)*/
  E_DHD_DRAW_6UVS       = DHD_DRAW_UV(6),         /* drawing should use six sets of UV corrdinates per primitive (texture units 0..5)*/
  E_DHD_DRAW_7UVS       = DHD_DRAW_UV(7),         /* drawing should use seven sets of UV corrdinates per primitive (texture units 0..6)*/
  E_DHD_DRAW_8UVS       = DHD_DRAW_UV(8),         /* drawing should use eight sets of UV corrdinates per primitive (texture units 0..7)*/

  E_DHD_DRAW_RHW        = DHD_DRAW_RHW(1),         /* drawing should use one set of RHW corrdinates per primitive*/
  E_DHD_DRAW_1RHWS      = DHD_DRAW_RHW(1),         /* drawing should use one set of RHW corrdinates per primitive*/
  E_DHD_DRAW_2RHWS      = DHD_DRAW_RHW(2),         /* drawing should use two sets of RHW corrdinates per primitive (texture units 0..1)*/
  E_DHD_DRAW_3RHWS      = DHD_DRAW_RHW(3),         /* drawing should use three sets of RHW corrdinates per primitive (texture units 0..2)*/
  E_DHD_DRAW_4RHWS      = DHD_DRAW_RHW(4),         /* drawing should use four sets of RHW corrdinates per primitive (texture units 0..3)*/
  E_DHD_DRAW_5RHWS      = DHD_DRAW_RHW(5),         /* drawing should use fife sets of RHW corrdinates per primitive (texture units 0..4)*/
  E_DHD_DRAW_6RHWS      = DHD_DRAW_RHW(6),         /* drawing should use six sets of RHW corrdinates per primitive (texture units 0..5)*/
  E_DHD_DRAW_7RHWS      = DHD_DRAW_RHW(7),         /* drawing should use seven sets of RHW corrdinates per primitive (texture units 0..6)*/
  E_DHD_DRAW_8RHWS      = DHD_DRAW_RHW(8),         /* drawing should use eight sets of RHW corrdinates per primitive (texture units 0..7)*/

  E_DHD_DRAW_COL        = DHD_DRAW_COL(1),         /* drawing should use one constant color per primitive*/
  E_DHD_DRAW_1COLS      = DHD_DRAW_COL(1),         /* drawing should use one constant color per primitive*/
  E_DHD_DRAW_2COLS      = DHD_DRAW_COL(2),         /* drawing should use two constant colors per primitive (colors 0..1)*/
  E_DHD_DRAW_3COLS      = DHD_DRAW_COL(3),         /* drawing should use three constant colors per primitive (colors 0..2)*/
  E_DHD_DRAW_4COLS      = DHD_DRAW_COL(4),         /* drawing should use four constant colors per primitive (colors 0..3)*/
  E_DHD_DRAW_5COLS      = DHD_DRAW_COL(5),         /* drawing should use fife constant colors per primitive (colors 0..4)*/
  E_DHD_DRAW_6COLS      = DHD_DRAW_COL(6),         /* drawing should use six constant colors per primitive (colors 0..5)*/
  E_DHD_DRAW_7COLS      = DHD_DRAW_COL(7),         /* drawing should use seven constant colors per primitive (colors 0..6)*/
  E_DHD_DRAW_8COLS      = DHD_DRAW_COL(8),         /* drawing should use eight constant colors per primitive (colors 0..7)    */
  E_DHD_DRAW_9COLS      = DHD_DRAW_COL(9),         /* drawing should use two constant colors per primitive (colors 0..8)*/
  E_DHD_DRAW_10COLS     = DHD_DRAW_COL(10),        /* drawing should use two constant colors per primitive (colors 0..9)*/
  E_DHD_DRAW_11COLS     = DHD_DRAW_COL(11),        /* drawing should use three constant colors per primitive (colors 0..10)*/
  E_DHD_DRAW_12COLS     = DHD_DRAW_COL(12),        /* drawing should use four constant colors per primitive (colors 0..11)*/
  E_DHD_DRAW_13COLS     = DHD_DRAW_COL(13),        /* drawing should use fife constant colors per primitive (colors 0..12)*/
  E_DHD_DRAW_14COLS     = DHD_DRAW_COL(14),        /* drawing should use six constant colors per primitive (colors 0..13)*/
  E_DHD_DRAW_15COLS     = DHD_DRAW_COL(15),        /* drawing should use seven constant colors per primitive (colors 0..14)*/

  E_DHD_DRAW_DEPTH      = 0x1000,                  /* drawing should use a depth gradient per primitive*/
  E_DHD_DRAW_STRIPE     = 0x2000,                  /* drawing should use psu stripe width and offset from dhd_prim_data_t (only array draw commands)*/
  E_DHD_DRAW_COMMONBOX  = 0x4000                   /* drawing should not update the bounding box as all primitives will share a common bounding box*/
...
typedef enum tagdhd_pixel_org {
  E_DHD_PIXEL_ORG_ARGB_8888     = 0 | DHD_PIXEL_ORG_FLAG,   /* 32 Bit pixel using A8, R8, G8, B8 layout*/
  E_DHD_PIXEL_ORG_RGBA_8888     = 1 | DHD_PIXEL_ORG_FLAG,   /* 32 Bit pixel using R8, G8, B8, A8 layout*/
  E_DHD_PIXEL_ORG_ARGB_4444     = 2 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using A4, R4, G4, B4 layout*/
  E_DHD_PIXEL_ORG_RGBA_4444     = 3 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using R4, G4, B4, A4 layout*/
  E_DHD_PIXEL_ORG_ARGB_1555     = 4 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using A1, R5, G5, B5 layout*/
  E_DHD_PIXEL_ORG_RGBA_5551     = 5 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using R5, G5, B5, A1 layout*/
  E_DHD_PIXEL_ORG_RGB_565       = 6 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using R5, G6, B5 layout */
  E_DHD_PIXEL_ORG_AL_88         = 7 | DHD_PIXEL_ORG_FLAG,   /* 16 Bit pixel using A8, L8 layout*/
  E_DHD_PIXEL_ORG_AL_44         = 8 | DHD_PIXEL_ORG_FLAG,   /* 8 Bit pixel using A4, L4 layout*/
  E_DHD_PIXEL_ORG_A_8           = 9 | DHD_PIXEL_ORG_FLAG,   /* 8 Bit pixel using alpha only*/
  E_DHD_PIXEL_ORG_L_8           = 10 | DHD_PIXEL_ORG_FLAG,  /* 8 Bit pixel using luminance only */
  E_DHD_PIXEL_ORG_AL_17         = 11 | DHD_PIXEL_ORG_FLAG,  /* 8 Bit pixel using A1, L7 layout*/
...
typedef enum tagdhd_clutmode_org {
  ...
  E_DHD_CLUT_INDEXED_COLOR = 1<<4,     /* texture blue channel is used as LUT index*/
  E_DHD_CLUT_MAP_RGB = 2<<4,           /* RGB channels are used as LUT indices */
  E_DHD_CLUT_MAP_ARGB = 3<<4,          /* ARGB channels are used as LUT indices*/
  E_DHD_CLUT_KERNEL = 4<<4,            /* lookup using a filter kernel from the CLUT (see dhd_set_texture_kernel)*/
  E_DHD_CLUT_KERNEL_SIGNED = 5<<4      /* lookup using a signed filter kernel from the CLUT (see dhd_set_texture_kernel)*/
...
typedef enum tagdhd_texel_org {
  E_DHD_TEXEL_ORG_ARGB_8888 = 0 | DHD_TEXEL_ORG_FLAG,    /* 32 Bit texel using A8, R8, G8, B8 layout*/
  E_DHD_TEXEL_ORG_RGBA_8888 = 1 | DHD_TEXEL_ORG_FLAG,    /* 32 Bit texel using R8, G8, B8, A8 layout*/
  E_DHD_TEXEL_ORG_ARGB_4444 = 2 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using A4, R4, G4, B4 layout*/
  E_DHD_TEXEL_ORG_RGBA_4444 = 3 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using R4, G4, B4, A4 layout*/
  E_DHD_TEXEL_ORG_ARGB_1555 = 4 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using A1, R5, G5, B5 layout*/
  E_DHD_TEXEL_ORG_RGBA_5551 = 5 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using R5, G5, B5, A1 layout*/
  E_DHD_TEXEL_ORG_RGB_565   = 6 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using R5, G6, B5 layout */
  E_DHD_TEXEL_ORG_AL_88     = 7 | DHD_TEXEL_ORG_FLAG,    /* 16 Bit texel using A8, L8 layout*/
  E_DHD_TEXEL_ORG_AL_44     = 8 | DHD_TEXEL_ORG_FLAG,    /* 8 Bit texel using A4, L4 layout*/
  E_DHD_TEXEL_ORG_AL_17     = 9 | DHD_TEXEL_ORG_FLAG,    /* 8 Bit texel using A1, L7 layout*/
  E_DHD_TEXEL_ORG_AL_8      = 10 | DHD_TEXEL_ORG_FLAG,   /* 8 Bit texel using a single value for all channels*/
  E_DHD_TEXEL_ORG_AL_4      = 11 | DHD_TEXEL_ORG_FLAG,   /* 4 Bit texel using a single value for all channels*/
  E_DHD_TEXEL_ORG_AL_2      = 12 | DHD_TEXEL_ORG_FLAG,   /* 2 Bit texel using a single value for all channels*/
  E_DHD_TEXEL_ORG_AL_1      = 13 | DHD_TEXEL_ORG_FLAG,   /* 1 Bit texel using a single value for all channels*/

  E_DHD_TEXEL_ORG_CLUT_88 = E_DHD_TEXEL_ORG_AL_88 | E_DHD_CLUT_INDEXED_COLOR,  /* 8 Bit texel used as index into the CLUT*/
  E_DHD_TEXEL_ORG_CLUT_44 = E_DHD_TEXEL_ORG_AL_44 | E_DHD_CLUT_INDEXED_COLOR,  /* 4 Bit texel used as index into the CLUT*/
  E_DHD_TEXEL_ORG_CLUT_8  = E_DHD_TEXEL_ORG_AL_8 | E_DHD_CLUT_INDEXED_COLOR,   /* 8 Bit texel used as index into the CLUT*/
  E_DHD_TEXEL_ORG_CLUT_4  = E_DHD_TEXEL_ORG_AL_4 | E_DHD_CLUT_INDEXED_COLOR,   /* 4 Bit texel used as index into the CLUT*/
  E_DHD_TEXEL_ORG_CLUT_2  = E_DHD_TEXEL_ORG_AL_2 | E_DHD_CLUT_INDEXED_COLOR,   /* 2 Bit texel used as index into the CLUT*/
  E_DHD_TEXEL_ORG_CLUT_1  = E_DHD_TEXEL_ORG_AL_1 | E_DHD_CLUT_INDEXED_COLOR,   /* 1 Bit texel used as index into the CLUT */
...
typedef enum tagdhd_depth_org {
  E_DHD_DEPTH_ORG_S8A8Z16 = 0 | DHD_DEPTH_ORG_FLAG,    /* 32 Bit pixel using S8, A8, Z16 layout*/
  E_DHD_DEPTH_ORG_S8A0Z24 = 1 | DHD_DEPTH_ORG_FLAG,    /* 32 Bit pixel using S8, Z24 layout without global alpha*/
  E_DHD_DEPTH_ORG_S1A5Z10 = 2 | DHD_DEPTH_ORG_FLAG,    /* 16 Bit pixel using S1, A5, Z10 layout*/
  E_DHD_DEPTH_ORG_S8A8Z0  = 3 | DHD_DEPTH_ORG_FLAG,    /* 16 bit pixel using S8, A8 layout */
  E_DHD_DEPTH_ORG_S1A0Z15 = 4 | DHD_DEPTH_ORG_FLAG,    /* 16 Bit pixel using S1, Z15 layout without global alpha*/
  E_DHD_DEPTH_ORG_S1A7Z0  = 5 | DHD_DEPTH_ORG_FLAG,    /* 8 Bit pixel using S1, A7 layout without depth*/
  E_DHD_DEPTH_ORG_S8A0Z0  = 6 | DHD_DEPTH_ORG_FLAG,    /* 8 Bit pixel using only stencil*/
  E_DHD_DEPTH_ORG_S0A8Z0  = 7 | DHD_DEPTH_ORG_FLAG     /* 8 Bit pixel using only global alpha*/
...
typedef enum tagdhd_alphamask_mode {
  E_DHD_ALPHAMASK_DISABLE        = (0 << 2) | (0 << 1) | 0,     /* disable global alpha processing*/
  E_DHD_ALPHAMASK_MULTIPLY       = (0 << 2) | (0 << 1) | 1,     /* multiply fragment alpha with global alpha*/
  E_DHD_ALPHAMASK_REPLACE        = (0 << 2) | (1 << 1) | 1,     /* replace fragment alpha with global alpha*/
  E_DHD_ALPHAMASK_MAXIMIZE       = (1 << 2) | (0 << 1) | 0,     /* update global alpha if fragment alpha is higher*/
  E_DHD_ALPHAMASK_CLEAR          = (2 << 2) | (0 << 1) | 0,     /* clear global alpha (fragment alpha is unchanged)*/
  E_DHD_ALPHAMASK_MULTIPLY_CLEAR = (2 << 2) | (0 << 1) | 1,     /* multiply fragment alpha with global alpha and clear global alpha*/
  E_DHD_ALPHAMASK_REPLACE_CLEAR  = (2 << 2) | (1 << 1) | 1,     /* replace fragment alpha with global alpha and clear global alpha*/
  E_DHD_ALPHAMASK_ANTIALIAS      = (3 << 2) | (0 << 1) | 0      /* update global alpha for antialiased polygon drawing*/
...
typedef enum tagdhd_wait_flags {
  ...
  E_DHD_WAIT_STC   =   1<<0,       /* Wait while stream controller is busy           */
  E_DHD_WAIT_STR   =   1<<1,       /* Wait while stream reader is busy           */
  E_DHD_WAIT_STW   =   1<<2,       /* Wait while stream writer is busy           */
  E_DHD_WAIT_REG   =   1<<3,       /* Wait while register file is busy           */
  E_DHD_WAIT_PSU   =   1<<4,       /* Wait while pixel selection is busy           */
  E_DHD_WAIT_TXI   =   1<<5,       /* Wait while stages after PSU up to ZSS are busy*/
  E_DHD_WAIT_TEX   =   1<<6,       /* Wait while stages after ZSS up to TXS are busy*/
  E_DHD_WAIT_COL   =   1<<7,       /* Wait while stages after TXS up to FBS are busy*/
  E_DHD_WAIT_BLEND =   1<<8,       /* Wait while stages after FBS up to FBC are busy*/
  E_DHD_WAIT_CLEAR =   1<<9,       /* Wait while clear unit is busy             */
  E_DHD_WAIT_WRITEBACK = E_DHD_WAIT_REG | E_DHD_WAIT_PSU | E_DHD_WAIT_TXI | E_DHD_WAIT_TEX | E_DHD_WAIT_COL | E_DHD_WAIT_BLEND /* Wait until all pixels are written (not including clear unit)*/
