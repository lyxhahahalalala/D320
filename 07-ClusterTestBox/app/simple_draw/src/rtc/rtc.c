...
******************************************************************************/
/******************************************************************************
Includes <System Includes> , 乬Project Includes乭
******************************************************************************/
...
void	init_rtc_RTCATINT1S( void );
void    RTCA_Update_Counter(uint32 time, uint32 Cal);
void    RTCA_Start_Value(uint32 time, uint32 Cal);
void    Read_RTCA_Counter(uint32 *time, uint32 *Cal);
void    Read_RTCA_SRBU(uint32 *count);
...
/******************************************************************************
* Function Name : void init_rtc_int( void )
* Description   : This function initializes the clock.
* Argument      : none
* Return Value  : none
******************************************************************************/
void init_rtca(void)
{
	...
	/* 12:00:00  16年11月4日星期五*/
	RTCA_Start_Value(0x00120000, 0x16110405);
...
void __DS3231_ReadTime(DATE *pTime)   // internal rtc
{
	uint32 time, cal;
...
void __DS3231_SetTime(DATE *pTime)   // internal rtc
{
	volatile uint32 time = 0, cal = 0;
...
{
   DS1302_ReadTime(pTime);                 //external rtc
}

void DS3231_SetTime(DATE *pTime)
{
    DS1302_SetTime(pTime);                   //external rtc
}
