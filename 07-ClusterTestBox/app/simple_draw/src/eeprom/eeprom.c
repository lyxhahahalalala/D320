...
const unsigned char Title[] = "MUIC";
...
unsigned int __EEROM_ReadWord(unsigned long dst) 
{
    return 0;//(ReadEEROM(dst));
}

void __EEROM_WriteWord(unsigned long dst, unsigned int val) 
{
    unsigned int dump = 0;
